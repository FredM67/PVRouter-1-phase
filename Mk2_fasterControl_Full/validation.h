/**
 * @file validation.h
 * @brief Compile-time validation of the user configuration.
 */
...
static_assert(!EMONESP_CONTROL || ((PRIORITY_ROTATION == RotationModes::PIN) && OVERRIDE_PIN_PRESENT), "******** Wrong configuration. Please check your config.h ! ********");
...
{
  return outputIndex < TOTAL_ROUTER_OUTPUTS;
}

constexpr uint32_t check_pins()
{
  ...
  if constexpr (TYPE_OF_DISPLAY == DisplayType::OLED)
  {
    ...
    for (const auto encoderPin : encoderPins)
    {
      ...
      if (bit_read(used_pins, encoderPin))
        return 0;

      bit_set(used_pins, encoderPin);
    }
  }
  ...
  for (uint8_t idx = 0; idx < boostControls.size(); ++idx)
  {
    ...
    if (bit_read(used_pins, boostControls[idx].inputPin))
      return 0;
  ...
  for (uint8_t idx = 0; idx < diversionGroups.size(); ++idx)
  {
    ...
    if (bit_read(used_pins, diversionGroups[idx].inputPin))
      return 0;

    bit_set(used_pins, diversionGroups[idx].inputPin);
  }

  return used_pins;
}

constexpr bool check_relay_pins()
{
  bool pins_ok{ true };
