/* balanceCheck_1kW.ino
 *
 * This sketch is based on the standard Mk2 code for my mature product.
 * 1 kW of surplus power is synthesised to falicitate a simple balance test
 * without the need for a second load to act as a PV generator.
 * With a small load, the output should remain on; with a load greater
 * than 1kW, it should cycle on/off in accordance with the "mode" switch
 * setting.
 *
 *  RAE, April 2014
 */
...
#define SURPLUS_PV_IN_WATTS 1000
...
long surplusPVperMainsCycle_inIEU;
...
void setup()
{
  ...
  surplusPVperMainsCycle_inIEU = (long)SURPLUS_PV_IN_WATTS * (1 / powerCal_grid);
  ...
  if (polarityNow == POSITIVE)
  {
    if (beyondStartUpPhase)
    {
      if (polarityOfLastSampleV != POSITIVE)
      {
        ...
        realPower_grid += surplusPVperMainsCycle_inIEU;  // <- for balance test
