...
void setup()
{
  ...
  Serial.println();
  Serial.println("-------------------------------------");
  Serial.println("Sketch ID:      cal_bothDisplays_3.ino");
  Serial.println();
  ...
  if (polarityNow == POSITIVE)
  {
    if (beyondStartUpPhase)
    {
      if (polarityOfLastSampleV != POSITIVE)
      {
        ...
        {
          perSecondCounter = 0;
          int powerInWatts_grid;
          int powerInWatts_diverted;

          powerInWatts_grid = energyThisSecond_grid / IEU_per_Joule_grid;
          powerInWatts_diverted = energyThisSecond_diverted / IEU_per_Joule_diverted;

          Serial.print("CT1: ");
          Serial.print(powerInWatts_grid);
          Serial.print("; CT2: ");
          Serial.println(powerInWatts_diverted);

          if (powerChannel == GRID)
          {
            configureValueForDisplay(powerInWatts_grid);
          }
          else
          {
            configureValueForDisplay(powerInWatts_diverted);
          }
