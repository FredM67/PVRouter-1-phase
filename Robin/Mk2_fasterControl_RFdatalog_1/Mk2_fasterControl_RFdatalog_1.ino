...
#define RF_SEND_PERIOD 5             // seconds
...
// For datalogging purposes, voltageCal has been included.  When running at
...
void allGeneralProcessing()
{
  ...
  static long sumP_forDivertedPower;
  static long sum_Vsquared;  // for summation of V^2 values during datalog period
      ...
      if (beyondStartUpPhase)
      {
        ...
        if (perSecondCounter > CYCLES_PER_SECOND)
        {
          ...
          if (RF_send_counter >= RF_SEND_PERIOD)
          {
            ...
            tx_data.powerAtSupplyPoint_Watts = realPower_long * powerCal_grid /*  * 16 */;
            tx_data.powerAtSupplyPoint_Watts *= -1;  // To match the OEM convention (so import is +ve)

            realPower_long = sumP_forDivertedPower / samplesDuringDatalogPeriod;
            tx_data.divertedPower_Watts = realPower_long * powerCal_diverted /* * 16 */;
  ...
  sumP_atSupplyPoint += instP;  // scaling is x1/16, for datalogging
  ...
  //
  //instP = instP >> 4;              // reduce to 16-bits (x1/16, or 2^-4) for more datalog range
  sumP_forDivertedPower += instP;  // scaling is x1/16, for datalogging
